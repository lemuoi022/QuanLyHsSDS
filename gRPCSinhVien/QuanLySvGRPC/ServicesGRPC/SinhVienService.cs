...
namespace QuanLySvGRPC.Services
{
    ...
    public class SinhVienService : SinhVienRPC.SinhVienRPCBase
    {
        ...
        public override Task<PageSinhVienReply> GetPageSinhVien(PageSinhVienRequest request, ServerCallContext context)
        {
            ...
            {
                ID = request.SvSearch.ID != 0 ? request.SvSearch.ID : (int?)null,
                ...
                idLopHoc = request.SvSearch.IdLopHoc != 0 ? request.SvSearch.IdLopHoc : (int?)null
            };
        ...
        public override Task<ListSinhVienTheoLopReply> ThongKeSinhVienTheoLop(SinhVienTheoLopRequest request, ServerCallContext context)
        {
            bool? gioiTinh = null;
            if (request.Gioitinh == 0) gioiTinh = null;
            else if (request.Gioitinh == 1) gioiTinh = false;
            else if (request.Gioitinh == 2) gioiTinh = true;
            var listSvLop = _quanLySv.ThongKeSinhVienTheoLop(gioiTinh);

            var reply = new ListSinhVienTheoLopReply();
            reply.SvTheoLop.AddRange(listSvLop.Select(sv => new SinhVienTheoLop
            {
                TenLop = sv.TenLop,
