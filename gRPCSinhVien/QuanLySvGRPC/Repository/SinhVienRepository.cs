...
namespace QuanLySvGRPC.Repository
{
    public class SinhVienRepository : ISinhVienRepository
    {
        ...
        public List<SinhVienTheoLopDTO> GetSinhVienCountByLopHoc(bool? gender = null)
        {
            using (var session = FluentNHibernateHelper.OpenSession())
            {

                var allLophoc = session.Query<LopHoc>().ToList();

                var query = from lh in allLophoc
                            join sv in session.Query<SinhVien>().Where(sv => gender == null || sv.GioiTinh == gender) on lh equals sv.LHoc into svGroup
                            from sv in svGroup.DefaultIfEmpty()
                            group sv by lh into g
                            select new SinhVienTheoLopDTO
                            {
                                TenLop = g.Key.TenLop,
                                SoSinhVien = g.Count(sv => sv != null)
                            };
